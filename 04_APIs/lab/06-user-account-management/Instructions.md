...
**Note:** The settings for project-level and app-level `urls.py` files have already been updated.

**Initial instructions**

Now run a command on the Terminal to initialize the pipenv environment such as:
...
Create the two attributes in the model and assign the respective form fields to them. Additionally, pass the following arguments to those form fields:
...
**Note:** The import statements required have already been added above. Additionally, the class `RatingSerializer` is already declared and updated with the relevant code. Take note of the `user` variable created and the contents of the code because you will be using it.
...
- Create a `fields` variable and assign a list to it that contains the following three string elements, in this order: `user`, `menuitem_id`, `rating`.
- Create a variable called `validators` and assign to it a list that contains:
  - A `UniqueTogetherValidator` class that has the following arguments passed to it:
    ...
    - a `fields` variable that contains two list elements that are strings: `user`, `menuitem_id`
- Create a variable called `extra_kwargs` that has a dictionary assigned to it containing one item with the following key-value pair:
  - Key: `rating`
...
![Add code to assign default renderer classes in the REST_FRAMEWORK section in the settings.py file.](./assets/rest_framework.png)

You will be adding a configuration inside `REST_FRAMEWORK` this time:
...
Now look for the `INSTALLED_APPS` setting inside the `settings.py` file and update the list with the following string elements:
...
Open the Terminal in VS Code and run both commands to perform the migrations.

**Step 10:**

Once the migrations are performed successfully, run the command to start the server on localhost and go to the URL: `http://127.0.0.1:8000/admin`.

Enter the details of the credentials for the superuser you created in step 7 and log into the Django admin panel. The Django Admin home page should appear as below:

![Django admin panel with the superuser login details.](./assets/user-account-management-img-2.png)
...
Press the **Save** button and add another user with the following details:
...
![Django admin panel for user changing](./assets/user-account-management-img-3.png)
...
![The user called Adrian appears in the user dropdown menu in the Add token section](./assets/user-account-management-img-4.png)
...
![Tokens for Mario and Adrian appears](./assets/user-account-management-img-5.png)
...
![A POST request in the API client Insomnia.](./assets/user-account-management-img-6.png)

Click on **Body** and select **FORM URL Encoded** as the data structure.

![FORM URL Encoded is an option in the dropdown menu that appears when you click on Body.](./assets/user-account-management-img-14.png)
...
![Details for data structure added](./assets/user-account-management-img-8.png)
...
![Token for Adrian added in the Bearer section.](./assets/user-account-management-img-9.png)

Press the **Send** button to generate the POST request. It will generate JSON output like in the screenshot below.

![JSON object created by a POST request.](./assets/user-account-management-img-10.png)

**Step 15:**

Now keep the value of `menuitem_id` unchanged but change the `rating` to `5`. It should generate a different JSON output than before, such as in the screenshot below. The error denotes the throttling limits you have placed on the number of reviews a user can add in a certain amount of time. It means that any given user can add at most one rating for a specific menu item, and a unique set must be created for a combination of a user and a menu item.

![JSON output that displays an error when a user tries to give a menu item another rating.](./assets/user-account-management-img-11.png)

**Step 16:**

Update the token to use the user Sana, and use the same values with which you encountered an error, such as `menuitem_id` with value `3` and a `rating` value of `5`. This should update the user review without any issue because it is now a new rating given by another user.

![JSON output of user 6 for menu item id 3 with a rating of 5.](./assets/user-account-management-img-12.png)

**Step 17:**

Switch the screen and go back to the webpage on your browser with the URL: `http://127.0.0.1:8000/api/ratings`.
